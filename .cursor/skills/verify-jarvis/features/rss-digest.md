...
- `rss-digest-retry-past-window` still retries a released Discord failure after the two-hour catch-up window.
...
- **Retry past window.** The test `retries a released Discord failure after the two-hour catch-up window` expects the second tick to publish the same item after a Discord failure, even once it is older than two hours.
