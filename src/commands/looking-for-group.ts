...
export interface LookingForGroupInteraction extends ReplyTarget {
  ...
  readonly user: Readonly<{ id: string; globalName?: string | null; username?: string }>;
...
export const handleLookingForGroupCommand = async (
  ...
  if (!interaction.guildId?.trim())
    return replySafely(interaction, 'This command is available only on the MuthaShip.', true);
  if (!dependencies.enabled || !dependencies.channelId.trim())
    return replySafely(interaction, 'Crew matchmaking is not configured on the MuthaShip.', true);
  ...
  if (!game) return replySafely(interaction, 'Tell me which game or activity you want crew for.', true);
  const name = interaction.user.globalName?.trim() || interaction.user.username?.trim() || 'Crew member';
  const suffix = [window && `When: ${window}`, details && `Details: ${details}`].filter(Boolean).join('\n');
  return replySafely(interaction, `📡 **Crew signal: ${name} is looking for a group**\nGame: ${game}${suffix ? `\n${suffix}` : ''}\n\nReply in this channel if you are joining.`, false);
};
