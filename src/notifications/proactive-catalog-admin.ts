import {
  buildProactiveCatalog,
  type ProactivePrompt,
} from './proactive-catalog.js';
...
export interface ProactiveCatalogAdmin {
  ...
  edit(
    id: string,
    patch: Partial<Pick<ProactivePrompt, 'category' | 'text' | 'active'>>,
...
export function createProactiveCatalogAdmin(
  initial: readonly ProactivePrompt[] = [],
): ProactiveCatalogAdmin {
  let catalog = buildProactiveCatalog(initial);
  const update = (next: readonly ProactivePrompt[]) =>
    (catalog = buildProactiveCatalog(next));
  return {
    list: () => catalog,
    ...
    setActive: (id, active) =>
      update(
        catalog.map((prompt) =>
          prompt.id === id ? { ...prompt, active } : prompt,
        ),
      ),
    retire: (id) =>
      update(
        catalog.map((prompt) =>
          prompt.id === id ? { ...prompt, active: false } : prompt,
        ),
      ),
  };
}
