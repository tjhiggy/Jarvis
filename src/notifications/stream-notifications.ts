...
export class StreamNotificationError extends Error {
  constructor(
    public readonly code: 'unavailable' | 'not-configured',
    message: string,
  ) {
    super(message);
...
const timeoutFetch = async (
  ...
  try {
    ...
    throw new StreamNotificationError(
      'unavailable',
  ...
  async fetch(feed: StreamFeed): Promise<readonly StreamNotification[]> {
    if (feed.provider === 'youtube') {
      const response = await timeoutFetch(
        `https://www.youtube.com/feeds/videos.xml?channel_id=${encodeURIComponent(feed.id)}`,
        this.timeoutMs,
      ...
      return [...xml.matchAll(/<entry>([\s\S]*?)<\/entry>/g)]
        .map((match) => {
          const item = match[1] ?? '';
          const id = decode(
            item.match(/<yt:videoId>([\s\S]*?)<\/yt:videoId>/)?.[1] ?? '',
          );
          return {
            id,
            provider: 'youtube' as const,
            title: decode(
              item.match(/<title>([\s\S]*?)<\/title>/)?.[1] ?? feed.label,
            ),
    ...
    const response = await timeoutFetch(
      `https://api.twitch.tv/helix/streams?user_login=${encodeURIComponent(feed.id)}`,
      this.timeoutMs,
      {
        headers: {
    ...
    return (body.data ?? []).map((item) => ({
      ...
      title: item.title,
      url: `https://twitch.tv/${encodeURIComponent(feed.id)}`,
      publishedAt: item.started_at,
...
export class StreamNotificationService {
  constructor(
    private readonly client: StreamNotificationClient,
    private readonly store: StreamNotificationStore,
    private readonly publisher: StreamNotificationPublisher,
  ...
  async poll(feeds: readonly StreamFeed[]): Promise<number> {
    ...
    for (const feed of feeds.slice(0, 20)) {
      ...
      try {
        items = await this.client.fetch(feed);
      } catch (error) {
        if (
          error instanceof StreamNotificationError &&
