...
export class SQLiteEngagementRepository implements EngagementRepository {
  ...
  async upsertRsvp(input: Rsvp): Promise<Rsvp> {
    ...
    this.database
      .prepare(
        `INSERT INTO engagement_rsvps (event_id, guild_id, user_id, response, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(event_id, user_id) DO UPDATE SET response = excluded.response, updated_at = excluded.updated_at`,
      )
      .run(
        value.eventId,
        value.guildId,
    ...
    const row = this.database
      .prepare(
        'SELECT * FROM engagement_rsvps WHERE event_id = ? AND user_id = ?',
      )
      .get(value.eventId, value.userId) as RsvpRow;
    return toRsvp(row);
  }
  ...
  async deleteOwnerData(guildId: string, userId: string): Promise<number> {
    ...
    return this.database.transaction(() => {
      ...
      const optOut = this.database
        ...
        .run(guildId, userId).changes;
      return introductions + suggestions + rsvps + optOut;
    })();
  }
  ...
  private migrate(): void {
    this.database.transaction(() => {
      ...
      if (
        this.database
          .prepare(
            'SELECT 1 FROM engagement_schema_migrations WHERE version = 1',
          )
          .get() !== undefined
      )
        return;
      this.database.exec(`
      CREATE TABLE engagement_introductions (id TEXT PRIMARY KEY, guild_id TEXT NOT NULL, channel_id TEXT NOT NULL, owner_user_id TEXT NOT NULL, display_name TEXT NOT NULL, interests TEXT NOT NULL, introduction TEXT NOT NULL, status TEXT NOT NULL CHECK (status IN ('active', 'deleted')), created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
      CREATE TABLE engagement_suggestions (id TEXT PRIMARY KEY, guild_id TEXT NOT NULL, channel_id TEXT NOT NULL, owner_user_id TEXT NOT NULL, title TEXT NOT NULL, description TEXT NOT NULL, status TEXT NOT NULL CHECK (status IN ('open', 'acknowledged', 'deferred', 'resolved', 'archived')), created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
      CREATE TABLE engagement_events (id TEXT PRIMARY KEY, guild_id TEXT NOT NULL, channel_id TEXT NOT NULL, owner_user_id TEXT NOT NULL, title TEXT NOT NULL, description TEXT NOT NULL, scheduled_at INTEGER NOT NULL, timezone TEXT NOT NULL, capacity INTEGER NOT NULL CHECK (capacity > 0), status TEXT NOT NULL CHECK (status IN ('scheduled', 'cancelled', 'completed')), created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL);
      CREATE TABLE engagement_rsvps (event_id TEXT NOT NULL REFERENCES engagement_events(id) ON DELETE CASCADE, guild_id TEXT NOT NULL, user_id TEXT NOT NULL, response TEXT NOT NULL CHECK (response IN ('yes', 'maybe', 'no')), created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, PRIMARY KEY (event_id, user_id));
      ...
      CREATE TABLE engagement_idempotency_keys (guild_id TEXT NOT NULL, scope TEXT NOT NULL CHECK (scope IN ('interaction', 'scheduled-job')), key TEXT NOT NULL, created_at INTEGER NOT NULL, PRIMARY KEY (guild_id, scope, key));
      ...
      CREATE INDEX engagement_events_status_scheduled ON engagement_events (status, scheduled_at, id);
      CREATE INDEX engagement_events_retention ON engagement_events (status, updated_at, id);
      CREATE INDEX engagement_rsvps_guild_user ON engagement_rsvps (guild_id, user_id, event_id);
      CREATE INDEX engagement_idempotency_retention ON engagement_idempotency_keys (created_at, key);
    `);
      this.database
        .prepare(
          'INSERT INTO engagement_schema_migrations (version, applied_at) VALUES (1, ?)',
        )
        .run(Date.now());
    })();
  }
