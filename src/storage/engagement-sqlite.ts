...
export class SQLiteEngagementRepository implements EngagementRepository {
  ...
  async recordParticipationStreak(
    guildId: string,
    userId: string,
    day: string,
    at: Date,
    ...
    const row = this.database
      .prepare(
        'SELECT last_day,current_streak,longest_streak FROM engagement_participation_streaks WHERE guild_id=? AND user_id=?',
      )
      .get(guildId, userId) as
      | { last_day: string; current_streak: number; longest_streak: number }
      | undefined;
    if (row?.last_day === day)
    ...
    const current =
      row?.last_day === previous.toISOString().slice(0, 10)
        ? row.current_streak + 1
        : 1;
    const longest = Math.max(row?.longest_streak ?? 0, current);
    this.database
      .prepare(
        'INSERT INTO engagement_participation_streaks (guild_id,user_id,last_day,current_streak,longest_streak,updated_at) VALUES (?,?,?,?,?,?) ON CONFLICT(guild_id,user_id) DO UPDATE SET last_day=excluded.last_day,current_streak=excluded.current_streak,longest_streak=excluded.longest_streak,updated_at=excluded.updated_at',
      )
      .run(guildId, userId, day, current, longest, milliseconds(at));
