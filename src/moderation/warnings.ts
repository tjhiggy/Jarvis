...
const id = /^[0-9]{8,20}$/;
export function createModerationWarning(
  input: ModerationWarning,
): ModerationWarning {
  for (const value of [
    input.serverId,
    input.warningId,
    ...
    if (!id.test(value))
      throw new Error('invalid moderation warning identifier');
  }
  const reason = input.reason.trim();
  if (
    !reason ||
    reason.length > 500 ||
    /<@(?:!|&)?\d{8,20}>|@(everyone|here)/i.test(reason)
  ) {
    throw new Error('invalid moderation warning reason');
  }
  const at = new Date(input.at);
  if (Number.isNaN(at.getTime()))
    throw new Error('invalid moderation warning timestamp');
