...
export function validateAutoModPolicy(policy: AutoModPolicy): AutoModPolicy {
  ...
  for (const rule of policy.rules) {
    if (seen.has(rule.kind)) throw new Error('duplicate AutoMod rule');
    if (
      !Number.isInteger(rule.threshold) ||
      rule.threshold < 1 ||
      rule.threshold > 100
    )
      throw new Error('invalid AutoMod threshold');
    seen.add(rule.kind);
  }
...
export function evaluateAutoMod(
  ...
  input: { readonly kind: AutoModRuleKind; readonly count: number },
): 'allow' | 'flag' {
  if (!policy.enabled) return 'allow';
  const rule = policy.rules.find((candidate) => candidate.kind === input.kind);
  return rule !== undefined && rule.enabled && input.count >= rule.threshold
    ? 'flag'
    : 'allow';
}
