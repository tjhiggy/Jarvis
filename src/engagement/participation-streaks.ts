...
export interface ParticipationStreakResult {
...
export class ParticipationStreakService {
  ...
  async record(
    ...
    userId: string,
    now = new Date(),
  ): Promise<ParticipationStreakResult> {
    const day = dayKey(now);
    if (await this.store.optedOut(serverId, userId))
      return { current: 0, longest: 0, day };
...
}): ParticipationStreakStore => ({
  record: repository.recordParticipationStreak.bind(repository),
