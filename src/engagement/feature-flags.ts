...
export class FeatureFlagService {
  ...
  async list(guildId: string): Promise<readonly FeatureFlagRecord[]> {
    const persisted = new Map(
      (await this.repository.getFeatureFlags(guildId)).map((entry) => [
        entry.name,
        entry.enabled,
      ]),
    );
    return SUPPORTED_FEATURE_FLAGS.map((name) => ({
      name,
      enabled: persisted.get(name) ?? defaultFor(name),
    }));
  }
  async isEnabled(guildId: string, name: FeatureFlagName): Promise<boolean> {
    if (!SUPPORTED_FEATURE_FLAGS.includes(name)) return true;
    const entry = (await this.repository.getFeatureFlags(guildId)).find(
      (value) => value.name === name,
    );
    return entry?.enabled ?? defaultFor(name);
  }
  async set(
    guildId: string,
    name: FeatureFlagName,
