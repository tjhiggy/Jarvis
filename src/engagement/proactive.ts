...
export const buildProactivePreview = (
  template = DEFAULT_PROACTIVE_TEMPLATES[0] ?? '',
): string => neutralizeDiscordMentions(template).slice(0, 1_000);
...
export class ProactiveEngagementService {
  ...
  async preview(template?: string): Promise<string> {
    return buildProactivePreview(
      template ??
        this.dependencies.templates?.[0] ??
        DEFAULT_PROACTIVE_TEMPLATES[0],
    );
  }
  ...
  async tick(): Promise<boolean> {
    ...
    if (
      !(await this.dependencies.store.claim(
        this.dependencies.guildId,
        key,
        now,
      ))
      ...
      return false;
    const templates = this.dependencies.templates?.length
      ? this.dependencies.templates
      : DEFAULT_PROACTIVE_TEMPLATES;
    await this.dependencies.gateway.post(
      this.dependencies.channelId,
      buildProactivePreview(
        templates[Math.floor(now.getTime() / 3_600_000) % templates.length],
      ),
    );
    if (this.dependencies.store.recordPosted)
