...
export class InMemorySupportTicketRepository implements SupportTicketRepository {
  ...
  async create(ticket: SupportTicket): Promise<void> {
    this.tickets.set(`${ticket.guildId}:${ticket.id}`, ticket);
  }
  async get(guildId: string, id: string): Promise<SupportTicket | undefined> {
    return this.tickets.get(`${guildId}:${id}`);
  }
  async listOpenForRequester(
    guildId: string,
    requesterId: string,
  ): Promise<readonly SupportTicket[]> {
  ...
  async listOpen(guildId: string): Promise<readonly SupportTicket[]> {
    return [...this.tickets.values()].filter(
...
export type SupportTicketResult =
  ...
  | {
      readonly ok: false;
      readonly reason:
        'disabled' | 'invalid' | 'limit' | 'not-found' | 'forbidden';
    };

export class SupportTicketService {
  constructor(
    private readonly repository: SupportTicketRepository,
    private readonly policy: SupportTicketPolicy,
  ) {}
  ...
  async create(input: {
    ...
    if (
      ...
      input.channelId !== this.policy.channelId
    )
      return { ok: false, reason: 'disabled' };
    const subject = input.subject.trim();
    if (
      subject.length < 1 ||
      subject.length > (this.policy.maxSubjectChars ?? 240)
    )
      return { ok: false, reason: 'invalid' };
    const open = await this.repository.listOpenForRequester(
      input.guildId,
      input.requesterId,
    );
    if (open.length >= (this.policy.maxOpenPerRequester ?? 1))
  ...
  async close(input: {
    ...
    const isAdmin = [...input.actorRoleIds].some((role) =>
      this.policy.adminRoleIds.has(role),
    );
    if (ticket.requesterId !== input.actorId && !isAdmin)
