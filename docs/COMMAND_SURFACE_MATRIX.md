...
| `/rss add`, `list`, `remove`, `pause`, `resume`                                    | Administrator          | Feed config and external delivery   | Command Deck primary, Discord fallback | Deck owns host validation, fixed destination, preview/confirm/cancel/retry/rollback, audit, and delivery metrics; Sites controls remain pre-release. |
