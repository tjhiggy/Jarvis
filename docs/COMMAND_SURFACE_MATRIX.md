...
| `/knowledge list`, `/knowledge approve`, `/knowledge revoke`                       | Administrator          | Persistent approval mutation        | Command Deck primary, Discord fallback | Keep the current immediate Discord fallback; migrate routine review and explicit confirmation to the Deck.                                                                             |
...
| `/introduce preview`, `confirm`, `cancel`; `/introduction id:<id>`                 | Member                 | Public card mutation                | Both                                   | Discord is primary; Deck provides moderation queue, retention, and deletion audit.                                                                                                     |
| `/suggest preview`, `confirm`, `cancel`; `/suggestion delete`                      | Member / administrator | Public card and moderation mutation | Both                                   | Discord remains fast; suggestion-card buttons provide administrator triage while the Deck owns bulk review and audit.                                                                  |
...
| `/event create`, `list`, `details`, `cancel`; `/game-night create`, `list`; `/lfg`; `/bird-call` | Member / administrator | Guided public workflow              | Both                                   | Discord owns participation; Deck owns configuration, scheduling health, and bulk recovery. `/bird-call` is the instant guild-channel ping and does not replace `/lfg` or `/game-night`. |
...
| `/rss add`, `list`, `remove`, `pause`, `resume`                                    | Administrator          | Feed config and external delivery   | Command Deck primary, Discord fallback | Deck owns host validation, fixed destination, preview/confirm/cancel/retry/rollback, audit, and delivery metrics; Sites controls stay disabled until tokens and origin are configured. |
