...
| `ADMIN_CONSOLE_TOKEN`                     | Required when `ADMIN_CONSOLE_ENABLED=true`; non-empty secret                                | Empty string                 | Bearer token for local Command Deck write actions such as RSS pause/resume.                                | `stored-in-secret-manager`             | Secret                 |
| `COMMAND_DECK_API_TOKEN`                  | Optional; blank disables the remote read API; at least 32 characters when set               | Empty string                 | Dedicated bearer token for the private Sites read API; must differ from the local write token.             | `stored-in-secret-manager`             | Secret                 |
| `COMMAND_DECK_API_ALLOWED_ORIGINS`        | Optional comma-separated exact HTTPS origins; requires the read token                       | Empty list                   | Allows only the private Sites server origin; paths, credentials, HTTP, and wildcards are rejected.         | `https://deck.example.com`             | Access boundary        |
...
| `JARVIS_VERSION`                          | Optional; non-empty when supplied                                                           | Package version              | Trusted release version shown in `/status` and runtime answers.                                            | `1.5.0`                                | Build metadata         |
...
| `GITHUB_OWNER`                            | Optional; required together with `GITHUB_REPO` to enable GitHub commands                    | Empty string                 | Approved GitHub owner or organization.                                                                     | `tjhiggy`                              | Public identifier      |
| `GITHUB_REPO`                             | Optional; required together with `GITHUB_OWNER` to enable GitHub commands                   | Empty string                 | Approved repository for read-only lookups.                                                                 | `Jarvis`                               | Public identifier      |
| `GITHUB_TOKEN`                            | Optional fine-grained read-only token                                                       | Empty string                 | Public lookups work without it; when supplied, grant metadata read only. Never logged or shown in Discord. | `stored-in-secret-manager`             | Secret                 |
