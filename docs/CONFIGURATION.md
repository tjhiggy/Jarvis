...
| `POLL_ADMIN_USER_IDS`       | Empty with secret to disable; otherwise one or more 17-to-20 digit IDs | Empty set                    | Exact Discord user IDs authorized to create and close polls.                                     | `12345678901234567,98765432109876543` | Access boundary        |
| `POLL_VOTER_SECRET`         | Empty with admin IDs to disable; otherwise at least 32 characters      | Empty string                 | Private HMAC key for anonymous poll voter tokens.                                                | `stored-in-secret-manager`            | Secret                 |
