...
Jarvis loads `.env` through `dotenv` during application startup when that file
is present. Copy `.env.example` to `.env` for local development; do not commit
`.env`. Hosted Compose does not require a repo-local `.env`: export the same
keys (or inject them from the secret manager) into the Compose process
environment. Compose passes only listed keys into the container and does not
use `env_file`. All settings are parsed at startup, so restart the process
after changing any value. `npm run register-commands` separately loads the
registration subset and must be rerun after changing `MAX_INPUT_CHARS`,
`FAQ_CATALOG_PATH`, FAQ catalog content, or command definitions. It is a host
or CI job; the runtime image does not include `tsx`.
...
`AI_PROVIDER=openai` makes `OPENAI_API_KEY` mandatory. The OpenAI model, timeout, and retry settings are then used by the Responses adapter. With `AI_PROVIDER=ollama`, an OpenAI key is not required; the process calls `OLLAMA_BASE_URL/api/chat` with the configured Ollama model. The loader accepts only HTTP or HTTPS base URLs and strips trailing slashes before use. Native Ollama defaults to `http://127.0.0.1:11434`. The local Compose profile defaults to `http://host.docker.internal:11434` so the container can reach host Ollama on Linux and Docker Desktop. The hosted overlay sets `AI_PROVIDER=openai` and omits `OLLAMA_*`. Do not publish a local Ollama endpoint to the public internet.
