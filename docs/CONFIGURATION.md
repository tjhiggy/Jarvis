...
| `GITHUB_REPO`                        | Optional; required together with `GITHUB_OWNER` to enable GitHub commands                   | Empty string                 | Approved repository for lookups and bounded feature-request intake.                                                              | `Jarvis`                               | Public identifier      |
| `GITHUB_TOKEN`                       | Optional fine-grained token                                                                 | Empty string                 | Public lookups work without it; `/feature-request` requires repository Issues read/write only. Never logged or shown in Discord. | `stored-in-secret-manager`             | Secret                 |
