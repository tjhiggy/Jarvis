...
| GitHub integration      | Reads metadata, issues, and pull requests from one configured repository. A separate administrator-only preview workflow may create one issue with fixed intake labels when a fine-grained token is configured; all other repository mutations remain out of scope.                                               | `src/github/`, `src/commands/handlers.ts`                                                                                 |
