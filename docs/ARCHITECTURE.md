...
## Command Deck read boundary

The private Sites frontend consumes a versioned projection, not Jarvis internal
...
| GitHub integration      | Reads metadata, issues, and pull requests from one configured repository. Administrator `/request` in captains-quarters can create one issue there from what/why/done fields. Other feedback stays in native GitHub Discussions and issue forms.                                                                  | `src/github/`, `src/commands/request.ts`, `src/commands/handlers.ts`                                                      |
...
Jarvis has no implemented shell, code-execution, arbitrary-file, general GitHub-write, Discord-administration, webhook-management, or autonomous-learning capability. The only GitHub mutation is administrator `/request` creating one issue in the configured repository from the what/why/done fields. It cannot edit or close issues, manage pull requests, change repository settings, or select another repository. The optional poll path creates and edits only Jarvis-owned poll messages; it cannot edit or delete other content, roles, channels, permissions, members, settings, or webhooks. Conversation history supplies request context only. It is not a training loop, and no request can grant the application new authority.
