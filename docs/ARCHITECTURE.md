...
| ----------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Application composition | Loads configuration, persona, and the approved FAQ catalog before constructing adapters or logging into Discord; runs conversation retention cleanup, starts reminder and optional poll maintenance after login, and closes resources on `SIGINT` or `SIGTERM`. | `src/index.ts` |
...
| Command definitions     | Defines eight core commands, including read-only `/fantasy standings`, and adds `/poll` and `/poll-close` only when polls are configured. | `src/commands/definitions.ts` |
...
| Sleeper integration     | Reads one configured public Sleeper league's rosters and user display names for read-only standings; missing or unavailable data is reported without guessing. | `src/sleeper/`, `src/commands/handlers.ts` |
