...
| Sleeper integration     | Reads one configured public Sleeper league's rosters, owner names, standings, and weekly matchup data for read-only responses; missing or unavailable data is reported without guessing. | `src/sleeper/`, `src/commands/handlers.ts` |
