...
| `/profile create`, `view`, `edit`, `hide`, `show`, `delete`   | Any human member while the `profiles` feature flag is enabled. Create, edit, and delete use private 15-minute previews. Confirmation stores only server ID, user ID, bounded bio/interests, visibility, and timestamps. Profile records persist until member or administrator deletion; routine engagement retention does not delete them.                                                                                                                                                    |
...
| `/game-night create`, `list`                                  | A low-friction gaming wrapper around the event system. Administrators provide only a game name and future local start; details, timezone, and capacity default safely. It posts the same RSVP card, uses the configured event channel, and stores no game-specific data beyond the event title/details.                                                                                                                                                                                       |
| `/lfg game:<game> [when] [details]`                           | Any member can post a concise crew signal for a game or activity. It uses the configured activity channel gate and does not create a retained record or require administrator permissions.                                                                                                                                                                                                                                                                                                    |
| `/bird-call [game]`                                           | Any member can post one public MuthaShip-voice invite asking who wants to game now. Optional `game` names the activity. Guild channels only; direct messages fail closed. Role mentions in `game` render as role pings; mass, user, and channel mentions stay neutralized. It does not use the activity-channel gate and does not replace `/lfg` or `/game-night`.                                                                                                                            |
...
| `/daily`                                                      | Shipped in v0.9. Any member may claim one bounded 10-coin reward per server day. The claim is server/member/day unique, stores no message content, and honors engagement opt-out.                                                                                                                                                                                                                                                                                                             |
| `/streak`                                                     | Shipped in v0.9. Shows the server-scoped current and longest UTC-day participation streak, advanced by successful daily participation and suppressed for opted-out members.                                                                                                                                                                                                                                                                                                                   |
| `/engagement status`, `metrics`, `pause`, `resume`, `delete`  | Status/metrics/pause/resume need administrator roles. Anyone deletes their own retained server data; an administrator may supply `user_id`. Metrics show aggregate command events from the last seven days, never message content or secrets. Pause audit holds only server ID, actor ID, operation, and timestamp.                                                                                                                                                                           |
