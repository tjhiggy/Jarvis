...
This is the operating contract for the shipped MuthaShip engagement features.
...
| `/event create`, `list`, `details`, `cancel` | Create/cancel require administrator roles. Events store IDs, title, description, start/end, timezone, capacity, status, message ID, destination-missed flag, and timestamps. RSVPs store event/guild/user IDs, yes/maybe/no response, confirmed/waitlisted attendance, reminder opt-in, and timestamps. |
...
| Weekly recap | 60 seconds | Runs only when schedule, recap enablement, and engagement state permit it. A daily lease prevents duplicates; unavailable source publishes nothing. |
| Trivia expiry/results | 15 seconds | Claims expired rounds and posts one aggregate card. Failure releases its lease for later safe recovery; a paused guild releases without posting. |
