...
}>({ snapshot: getCommandDeckSnapshot(), source: 'sample' });
    ...
    <>
      <div className="banner">
        <div>
          ...
          <p>
            {source === 'live'
              ? 'Jarvis published this bounded snapshot. No Discord content or credentials are included.'
              : source === 'unavailable'
                ? 'The live snapshot is unavailable. Use the localhost Command Deck at 127.0.0.1:8787 until Jarvis recovers.'
                : 'This is a sample snapshot. Configure the private Sites read path to show live Jarvis identity.'}
          </p>
        </div>
...
export function CommandDeckApp({
  snapshot = getCommandDeckSnapshot(),
  source = 'sample',
}: {
  snapshot?: CommandDeckSnapshot;
  source?: CommandDeckPresentationSource;
  ...
  const connectionLabel =
    source === 'live'
      ...
      : source === 'unavailable'
        ...
        : 'Sample snapshot';
  return (
    <DeckContext.Provider value={{ snapshot, source }}>
