...
describe('proactive catalog admin boundary', () => {
  it('supports bounded list, edit, activation, and retirement', () => {
    const admin = createProactiveCatalogAdmin([{ id: 'welcome', category: 'community', text: 'Welcome aboard.', active: true }]);
    expect(admin.list()).toHaveLength(1);
    expect(admin.edit('welcome', { text: 'Welcome to the MuthaShip.' })[0]?.text).toContain('MuthaShip');
  ...
  it('rejects unsafe additions through the shared catalog validator', () => {
    ...
    expect(() => admin.add({ id: 'bad', category: 'x', text: '@everyone', active: true })).toThrow();
  });
});
