...
import {
  ...
  type ScheduledNotificationPublisher,
} from '../src/notifications/scheduled-notification.js';

describe('ScheduledNotificationService', () => {
  it('claims and publishes each item once', async () => {
    const seen = new Set<string>();
    const published: string[] = [];
    const store: ScheduledNotificationStore = {
      isPaused: async () => false,
      claim: async (key) => !seen.has(key) && (seen.add(key), true),
    };
    const publisher: ScheduledNotificationPublisher = {
      publish: async (_channel, item) => {
