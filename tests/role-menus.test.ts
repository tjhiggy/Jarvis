...
import {
  parseRoleMenuConfig,
  roleMenuSelection,
} from '../src/engagement/role-menus.js';

describe('self-service role menus', () => {
  it('parses bounded configured role choices', () => {
    const menus = parseRoleMenuConfig(
      'games:Games:123456789012345678,pc:PC:234567890123456789',
    );
  ...
  it('rejects malformed or duplicate role choices', () => {
    ...
    expect(() =>
      parseRoleMenuConfig(
        'games:Games:123456789012345678,games:Other:234567890123456789',
      ),
    ).toThrow();
  });
