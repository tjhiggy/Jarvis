...
import {
  ...
  redactModerationWarning,
} from '../src/moderation/warnings.js';
...
describe('moderation warnings', () => {
  const base = {
    ...
    actorId: '12345680',
    subjectId: '12345681',
    reason: 'Please keep discussion on topic.',
  ...
  it('rejects unbounded or mention-bearing reasons', () => {
    expect(() =>
      createModerationWarning({ ...base, reason: '@everyone stop' }),
    ).toThrow();
    expect(() =>
      createModerationWarning({ ...base, reason: 'x'.repeat(501) }),
    ).toThrow();
  });
});
