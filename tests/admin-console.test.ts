...
describe('admin console', () => {
  ...
  it('previews an RSS feed only with the local admin token and never persists it', async () => {
    ...
    expect((await fetch(endpoint, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ url: 'https://news.example/feed.xml' }) })).status).toBe(401);
    const response = await fetch(endpoint, { method: 'POST', headers: { authorization: 'Bearer secret', 'content-type': 'application/json' }, body: JSON.stringify({ url: 'https://news.example/feed.xml' }) });
    expect(response.status).toBe(200);
    expect(await response.json()).toEqual({ url: 'https://news.example/feed.xml', items: [{ title: 'Xbox update', url: 'https://news.example/feed.xml', publishedAt: '2026-08-10T12:00:00Z' }] });
